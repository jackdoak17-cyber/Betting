...
Output (stdout and OUT_FILE env path):
  - Sections for 80% and 100% (configurable via MODE).
  - For each fixture, concise lines like:
      Team — SOT 3+, Corners 2+, Shots 10+, Cards 1+
...
STATS = [s.strip().lower() for s in os.getenv(
    "STATS",
    "shots_on_target,corners,shots,cards_total"
).split(",") if s.strip()]
...
# Ordering if user includes many stats
...
def pick_team_name(side_block: Dict[str, Any], meta: Dict[str, Any], side: str) -> str:
    # prefer explicit name, else fall back to fixture names
...
def format_team_line(stats_block: Dict[str, Any], pct_key: str, chosen_stats: List[str], include_zero: bool) -> str:
    """
    Build a single team line such as:
      "SOT 3+, Corners 2+, Shots 9+, Cards 1+"
    using the *min* bound of pct_key ("p80" or "p100").
    """
    parts: List[str] = []
...
def main():
    ...
    # Group by fixture id to print once per fixture
    by_fix: Dict[Tuple[int, str, str, int], Dict[str, Any]] = {}
    for r in all_rows:
        meta = r["meta"]
        ...
        grp = by_fix.setdefault(key, {"meta": meta, "teams": []})
        grp["teams"].append(r)
    ...
    def do_section(pct_key: str, title: str):
        lines.append(f"===== {title} =====")
        for (fid, home, away, useN), grp in sorted(by_fix.items(), key=lambda x: (x[0][1].lower(), x[0][2].lower())):
            if fid == -1:
                fixture_hdr = f"{home} vs {away}"
            else:
                fixture_hdr = f"{home} vs {away}  [Fixture {fid}]"
            suffix = f"  (using last {useN})" if useN else ""
            lines.append(f"{fixture_hdr}{suffix}")
            # ensure home appears first in printing
            ...
            lines.append("")  # blank after each fixture
        lines.append("")      # blank after section

    if MODE in ("both", "p80"):
        do_section("p80", "Bet Builder — 80%")
    if MODE in ("both", "p100"):
        do_section("p100", "Bet Builder — 100%")

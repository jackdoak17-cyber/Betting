...
"""
Verify coverage against predicted_xi teams.
...
Writes:
  - data/team_stats/coverage.txt
  - data/team_stats/coverage.json
"""
...
OUT_TXT  = ROOT / "data" / "team_stats" / "coverage.txt"
OUT_JSON = ROOT / "data" / "team_stats" / "coverage.json"

STAT_KEYS = [
    "shots_total_last_n",
    "shots_on_target_last_n",
    "fouls_last_n",
    "tackles_last_n",
    "cards_total_last_n",
    "saves_last_n",
...
def main():
    ...
    for lid in leagues:
        ...
        txt_lines.append(f"Present in team_stats : {present}")
        for k in STAT_KEYS:
            label = k.replace("_last_n", "")
            txt_lines.append(f"Non-empty {label:<20}: {per_stat_non_empty[k]}")
        if missing:

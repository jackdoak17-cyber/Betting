...
# -*- coding: utf-8 -*-
"""
Build per-player tackles histories (last N league matches of CURRENT season, ≥45').
Targets ONLY players found in data/predicted_xi/by_league/*.json.
Writes:
  - data/player_tackles/by_league/{league_id}.json
  - data/player_tackles/combined.json
  - data/player_tackles/summary.txt
"""
...
def fetch_team_fixtures_window(team_id: int, start: dt.date, end: dt.date, league_id: int, page: int = 1) -> dict:
    ...
    Only lineups.details for TACKLES + MINUTES.
    """
    path = f"fixtures/between/{dstr(start)}/{dstr(end)}/{team_id}"
    params = {
        "include": "lineups.details;lineups.player;state",
        "filters": f"lineupDetailTypes:{TACKLES},{MINUTES_PLAYED};fixtureLeagues:{league_id}",
        "order": "desc",
        "per_page": 50,
        "page": page,
    }
...
def tackles_from_details(details: List[dict]) -> int:
    for d in details or []:
        if int(d.get("type_id") or 0) == TACKLES:
...
def collect_player_series_for_team(league_id: int, season_id: int, team_id: int, targets: Dict[int, dict]) -> Dict[int, dict]:
    """
    Returns: player_id -> { 'tackles': [latest->older], 'minutes': [...], 'fixtures': [ids] }
    (Only fixtures in this league & season; include only if minutes >= MIN_MINUTES)
    """
    want = set(targets.keys())
    series: Dict[int, dict] = {pid: {"tackles": [], "minutes": [], "fixtures": []} for pid in want}

    start_season, end_today = get_season_bounds(season_id)
    end = end_today
    have_all = lambda: all(len(series[pid]["tackles"]) >= LAST_N for pid in want)
    ...
    while end >= start_season and not have_all():
        ...
        while has_more and not have_all():
            ...
            for fx in data:
                ...
                if st not in (5,):  # finished only (examples use 5)
                    continue
                ...
                for lp in fx.get("lineups") or []:
                    ...
                    if mins is None or mins < MIN_MINUTES:
                        continue
                    tkl = tackles_from_details(details)
                    s = series[pid]
                    s["tackles"].append(int(tkl))
    ...
    for pid in series:
        series[pid]["tackles"] = series[pid]["tackles"][:LAST_N]
    ...
    for idx, ((league_id, season_id, team_id), targets) in enumerate(items, 1):
        ...
        for pid, s in series.items():
            entry = {
                ...
                "tackles_last_n": s["tackles"],   # latest first

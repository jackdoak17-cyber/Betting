...
- 80%:  min_expected = min(team_off_80,  opp_allw_80)
        max_expected = max(team_off_80,  opp_allw_80)
- 100%: min_expected = min(team_off_100, opp_allw_100)
        max_expected = max(team_off_100, opp_allw_100)

Inputs
------
- data/predicted_xi/by_league/{league_id}.json  (fixtures list with team ids/names)
...
def _team_index(blob: dict, is_opponent: bool) -> Dict[int, dict]:
    """
    Build team_id -> row dict for quick lookup.
    Input blobs have 'teams': [ { team_id, team_name, ... series keys ... } ]
    """
    out: Dict[int, dict] = {}
...
def _team_name_map_from_px(lid: int) -> Dict[int, str]:
...
def threshold_at_pct(seq: List[int], pct: float) -> int:
    """
    Return largest integer t such that coverage(seq, t) >= pct.
    If seq empty -> 0.
    """
    if not seq:
        return 0
...
def _pair_lines(off_seq: List[int], allw_seq: List[int]) -> Dict[str, Dict[str, int]]:
    """
    For p in {0.8, 1.0} compute team/offense and opponent/allowed thresholds and min/max pair.
    Returns:
      {
        "p80": {"off": int, "opp": int, "min_expected": int, "max_expected": int},
        "p100": {...}
      }
    """
    ...
    return {
        "p80": {
            "off": off80,
            "opp": opp80,
            "min_expected": min(off80, opp80),
            "max_expected": max(off80, opp80),
        },
        "p100": {
            "off": off100,
            "opp": opp100,
            "min_expected": min(off100, opp100),
            "max_expected": max(off100, opp100),
        },
    }
...
def build_for_league(lid: int) -> Optional[dict]:
    ...
    ops_path = OPS_DIR / f"{lid}.json"
    px_path  = PX_DIR  / f"{lid}.json"

    ts = _load_json(ts_path) or {}
    ops = _load_json(ops_path) or {}
    px  = _load_json(px_path) or {}

    if not px.get("fixtures"):
        print(f"[warn] No fixtures in {px_path}")
        return None

    ts_idx  = _team_index(ts, is_opponent=False)
    ops_idx = _team_index(ops, is_opponent=True)

    out_fixtures: List[dict] = []
    missing_off = set()
    missing_opp = set()

    fixtures = _fixtures_from_px(lid)
    for fx in fixtures:
        ...
        def side(team_id: int, opp_id: int) -> Dict[str, Any]:
            ...
            ores = ops_idx.get(opp_id)   # opponent-allowed is keyed by the opponent team id
...
def write_outputs(all_payloads: List[dict]) -> None:
    ...
    for pl in all_payloads:
        ...
        for fx in pl["fixtures"]:
            hn, an = fx.get("home_name") or f"H{fx['home_id']}", fx.get("away_name") or f"A{fx['away_id']}"
            fid = fx.get("fixture_id")
            lines.append(f"{hn} vs {an}  [F{fid}]")
            for side_name in ("home", "away"):
                ...
                for key, (_, _, label) in STAT_MAP.items():
                    ...
                    lines.append(
                        f"    - {label}: "
                        f"80% off={p80['off']}, opp={p80['opp']} ⇒ min={p80['min_expected']}, max={p80['max_expected']} | "
                        f"100% off={p100['off']}, opp={p100['opp']} ⇒ min={p100['min_expected']}, max={p100['max_expected']}"
                    )

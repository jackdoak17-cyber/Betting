...
"""
Coverage report for tackles histories vs predicted XI targets.
...
Writes:
  - data/player_tackles/coverage.txt
  - data/player_tackles/coverage.json
"""
import json
...
TACK_DIR  = Path("data/player_tackles/by_league")
OUT_TXT   = Path("data/player_tackles/coverage.txt")
OUT_JSON  = Path("data/player_tackles/coverage.json")
...
def _tackles_by_league() -> Dict[int, Dict[int, dict]]:
    idx_by_l: Dict[int, Dict[int, dict]] = {}
    for p in TACK_DIR.glob("*.json"):
        blob = _load_json(p) or {}
        lid = int(blob.get("league_id") or p.stem)
        idx: Dict[int, dict] = {}
...
def main():
    ...
    for lid in sorted(predicted.keys()):
        ...
        for row in preds:
            ...
            if data:
                present += 1
                series = data.get("tackles_last_n") or []
        ...
        txt_lines.append(f"Found in tackles file: {present}")

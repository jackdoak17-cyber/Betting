...
- data/player_shots/by_league/*.json   (expects 'players' with 'shots_last_n')
and team names from:
...
def _role(row: dict) -> str:
    ...
    if isinstance(tag, str) and tag.strip():
        return tag.strip()
    # broad bucket fallback
    pid = row.get("position_id")
    if isinstance(pid, int):
        return {24: "GK", 25: "DEF", 26: "MID", 27: "FWD"}.get(pid, "")
    return ""

def main():
    lines: List[str] = []
    for path in sorted(glob.glob(str(SHOTS_DIR / "*.json")), key=lambda p: int(Path(p).stem)):
        ...
        # build team name map from predicted XI

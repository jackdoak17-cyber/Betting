...
Adds optional H2H annotation:
  If data/h2h/{minId}_{maxId}.json exists, append:
    " | H2H last2: O2.5 x/2, BTTS y/2"
to Over 2.5 and BTTS rows.

(Everything else identical to your latest working version.)
"""
...
O25_PRIORITY = ["alternative total goals","total goals","goals over/under","goals over under"]
BTTS_STANDALONE_MDS = {"both teams to score","both teams to score?","btts","btts (yes/no)","both teams to score - yes/no"}
BTTS_TEAMS_TO_SCORE_MD = "teams to score"
...
def best_price_o25(rows: List[dict]) -> Optional[float]:
    for md in O25_PRIORITY:
        ...
        for r in rows:
            ...
            if rmd != md: continue
            if not is_full_time(rmd): continue
            ln = numeric_line_from_row(r)
            if (ln is None) or abs(ln - 2.5) > 1e-6: continue
            if not text_has_over(r): continue
            p = price_of(r)
...
def h2h_cache_path_for_ids(a: int, b: int) -> Path:
    lo, hi = (a,b) if a <= b else (b,a)
    return H2H_DIR / f"{lo}_{hi}.json"

def read_h2h_summary_for_fixture(fx: dict) -> Optional[Tuple[int,int,int,int]]:
    hid, aid = extract_team_ids_from_fixture(fx)
    if not (isinstance(hid,int) and isinstance(aid,int)):
        return None
    p = h2h_cache_path_for_ids(hid, aid)
    if not p.exists():
        return None
    try:
        j = json.loads(p.read_text(encoding="utf-8"))
        s = j.get("summary") or {}
        o25_hits = int(s.get("o25_hits", 0))
        o25_n    = int(s.get("o25_n", 0))
        btts_hits= int(s.get("btts_hits", 0))
        btts_n   = int(s.get("btts_n", 0))
        return (o25_hits, o25_n, btts_hits, btts_n)
    except Exception:
        return None
...
def main():
    ...
    flags_over25 = []  # (fixture, price, home%, away%, combo%, h2h_tuple_or_None, raw_fx)
    flags_btts   = []  # (fixture, price, home%, away%, combo%, h2h_tuple_or_None, raw_fx)
    flags_team15 = []  # (fixture, side, team, price, team%, ml_price, raw_fx)

    near_o25 = near_btts = near_t15 = 0
        ...
        for fx in fixtures:
            ...
            if pass_over25:
                ...
                if p is not None and p >= MIN_PRICE:
                    ...
                    flags_over25.append((name, p, h_over25[2], a_over25[2], combo, read_h2h_summary_for_fixture(fx), fx))
                elif p is not None:
                    near_o25 += 1
            ...
            if pass_btts:
                ...
                if p is not None and p >= MIN_PRICE:
                    ...
                    flags_btts.append((name, p, h_btts[2], a_btts[2], combo, read_h2h_summary_for_fixture(fx), fx))
                elif p is not None:
                    near_btts += 1

            # Team O1.5 (favourites only)
            if pass_h15 and is_home_fav:
                p = best_price(rows, lambda r: is_team_over15_row(r, "home"))
                if p is not None and p >= MIN_PRICE:
                    flags_team15.append((name, "home", home, p, h_o15[2], home_ml, fx))
                elif p is not None:
                    near_t15 += 1
            if pass_a15 and is_away_fav:
                p = best_price(rows, lambda r: is_team_over15_row(r, "away"))
                if p is not None and p >= MIN_PRICE:
                    flags_team15.append((name, "away", away, p, a_o15[2], away_ml, fx))
                elif p is not None:
                    near_t15 += 1
    ...
    for name, price, hp, ap, combo, h2h, _fx in flags_over25:
        tail = ""
        if h2h and h2h[1] > 0:
            o25_hits,o25_n,btts_hits,btts_n = h2h
            tail = f" | H2H last2: O2.5 {o25_hits}/{o25_n}"
    ...
    for name, price, hp, ap, combo, h2h, _fx in flags_btts:
        tail = ""
        if h2h and h2h[3] > 0:
            o25_hits,o25_n,btts_hits,btts_n = h2h
            tail = f" | H2H last2: BTTS {btts_hits}/{btts_n}"
    ...
    for name, side, team, price, tp, ml, _fx in flags_team15:
    ...
    # Markdown (same H2H notes if present)
    md = []

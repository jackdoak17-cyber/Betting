...
# -*- coding: utf-8 -*-
"""
Team Shots & Shots on Target — V2 (model-based shortlist)
...
4) Stabilise each hit rate toward the league baseline hit rate for that stat and
   threshold to avoid extreme values from small samples.
5) Blend stabilised rates into one model Over probability:
   - If H2H available: 50% team + 30% opponent + 20% H2H.
   - Otherwise: 60% team + 40% opponent.
6) Adjust for favourite/underdog using implied 1X2 win probability:
...
7) Convert odds to implied probabilities, compute edge (model − implied) and EV.
8) Keep any Over/Under side with positive EV and edge ≥0.05, provided team and
   opponent samples are not tiny (aim for 4+ same-venue games) and the hits are
   not one-off outliers.
...
Output:
- data/value_bets/reports/team_shots_sot_v2.txt (printed to console as well)
...
OUT_FILE  = REPORT_DIR / "team_shots_sot_v2.txt"
SHEET_FILE = SHEET_DIR / "team_shots_sot_v2_bets.csv"
# Legacy path before the sheets/ split; used to backfill missing rows if present
LEGACY_SHEET_FILE = BASE_OUT / "team_shots_sot_v2_bets.csv"
...
def reverse_fixture_value(
    ...
    Accept only fixtures roughly 200–500 days before kickoff to avoid pulling
    multiple-seasons-old or same-season meetings.
    """
...
def stabilise(rate: float, baseline: float, sample_size: int, k: float = 4.0) -> float:
    return (rate * sample_size + baseline * k) / (sample_size + k) if (sample_size + k) > 0 else baseline
...
def bet_key(row: dict) -> Tuple[str, str, str, str, str, str]:
    """Dedupe key that survives legacy rows without fixture/team IDs."""
    ...
    line_part = _normalize_line_key(row.get("line") or "")
    pick_part = (row.get("pick") or "").lower().strip()
    return (fixture_part, kickoff_part, team_part, stat_part, line_part, pick_part)

def load_sheet(path: Path, legacy_paths: Optional[List[Path]] = None) -> List[dict]:
    rows: List[dict] = []
    seen: set = set()

    for candidate in [path] + (legacy_paths or []):
...
def update_bet_sheet(path: Path, candidates: List[dict], ts_index: Dict[int, Dict[int, dict]]):
    existing = load_sheet(path, legacy_paths=[LEGACY_SHEET_FILE])
...
def league_offense_pool(ts_blob: dict) -> Dict[str, List[int]]:
    pools = {"shots_total": [], "shots_on_target": []}
    for team in (ts_blob.get("teams") or []):
        for stat, key in (("shots_total", "shots_total_last_n"), ("shots_on_target", "shots_on_target_last_n")):
            for v in team.get(key) or []:
                if isinstance(v, int):
                    pools[stat].append(v)
    return pools

def baseline_rate(pools: Dict[str, List[int]], stat: str, threshold: int) -> float:
    vals = pools.get(stat) or []
    if not vals:
        return 0.5
...
def main():
    ...
    for lid in league_ids:
        ...
        pools = league_offense_pool(ts_blob)
        ...
        for fx in fixtures:
            ...
            for (side, stat, line), prices in odds_map.items():
                ...
                base = baseline_rate(pools, stat, threshold)
                team_adj = stabilise(team_rate, base, team_total)
                opp_adj  = stabilise(opp_rate, base, opp_total)
                h2h_adj  = stabilise(h2h_rate, base, h2h_total) if h2h_total >= 2 else None

                if h2h_adj is not None:
                    over_p = 0.5 * team_adj + 0.3 * opp_adj + 0.2 * h2h_adj
                else:
                    over_p = 0.6 * team_adj + 0.4 * opp_adj
    ...
    candidates.sort(key=lambda r: (-r["ev"], r["kickoff"], r["fixture"], r["team"], r["market"], r["line"], r["pick"]))
    ...
    lines.append("Workflow: same-venue last10; line→threshold; hit-rate gates (Over ≥70%/Under ≤30% + reverse fixture hit); stabilised vs league; blend team/opponent/H2H; favourite tilt + moneyline guards; keep EV>0 & edge≥0.05 (samples ≥4).")
    lines.append("")
    lines.append("===== TEAM SHOTS & SOT (V2) =====")
    ...
    if not candidates:
        ...
        for r in candidates:
            ...
            lines.append(
                f"    model={r['model_p']:.3f} | implied={r['implied_p']:.3f} | edge={r['edge']:.3f} | EV={r['ev']:.3f} | threshold≥{r['threshold']}"
            )

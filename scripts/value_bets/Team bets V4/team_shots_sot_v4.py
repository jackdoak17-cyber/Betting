...
# -*- coding: utf-8 -*-
"""
Team Shots & Shots on Target — V4 (baseline-free, asymmetric hit-rate gates)
...
3) Compute hit rates for the threshold and require conversion: Overs need strong
   hit rates; Unders need weak hit rates. When available, H2H must have landed
   in the reverse fixture last season (same home/away orientation).
   - Overs: require at least one of {team attack, opponent concession} ≥70% and
     the other ≥50%; H2H ≥50% when at least 2 samples exist, and last season's
     reverse fixture hit the over.
   - Unders: require at least one of {team attack, opponent concession} ≤30% and
     the other ≤50%; H2H ≤50% when at least 2 samples exist, and last season's
     reverse fixture hit the under.
...
Output:
- data/value_bets/reports/team_shots_sot_v4.txt (printed to console as well)
...
OUT_FILE  = REPORT_DIR / "team_shots_sot_v4.txt"
SHEET_FILE = SHEET_DIR / "team_shots_sot_v4_bets.csv"
# Legacy path before the sheets/ split; used to backfill missing rows if present
LEGACY_SHEET_FILE = BASE_OUT / "team_shots_sot_v4_bets.csv"
...
def reverse_fixture_value(
    ...
    We only accept reverse fixtures between ~200 and ~500 days before the
    upcoming kickoff to ensure the match was last season, not several seasons
    ago or earlier in the current campaign.
    """
...
def bet_key(row: dict) -> Tuple[str, str, str, str, str, str]:
    ...
    Historic sheets lost fixture_id/team_id when the folder move happened. Using
    fixture text and kickoff as fallbacks prevents those rows from collapsing
    into one entry during merges.
    """
...
def main():
        ...
        for fx in fixtures:
            ...
            for (side, stat, line), prices in odds_map.items():
                ...
                for pick_side, model_p, implied_p, price in (
                    ...
                    if pick_side == "Over":
                        over_ok = (
                            (team_rate >= 0.70 and opp_rate >= 0.50)
                            or (opp_rate >= 0.70 and team_rate >= 0.50)
                        )
                        if not over_ok:
                            continue
                        ...
                        under_ok = (
                            (team_rate <= 0.30 and opp_rate <= 0.50)
                            or (opp_rate <= 0.30 and team_rate <= 0.50)
                        )
                        if not under_ok:
                            continue
                        if h2h_total >= 2 and h2h_rate > 0.50:
    ...
    lines.append("Workflow: same-venue last10; line→threshold; asymmetric hit-rate gates (Over needs 70%/50% combo, Under needs 30%/50% combo + reverse fixture hit); favourite tilt + moneyline guards; ranked by strongest model probability (samples ≥4).")
    lines.append("")
    lines.append("===== TEAM SHOTS & SOT (V4) =====")

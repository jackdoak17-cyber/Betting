...
# -*- coding: utf-8 -*-
"""
Team Bets V4 — BET RUN 3 (Team shots & shots on target overs, opponent-leaning)

Flip of BET RUN 2: opponent conceding signal is strong; team attack signal is
medium. Overs only, price gate >= 1.72.

Signals:
  - Team (medium) hit-rate gates: 6/9, 6/10, 8/12, 9/14, 11/17, 12/20 (no venue split).
  - Opponent (strong) allow gates: 8/9, 8/10, 10/12, 11/13, 12/14, 13/15, 15/19, 16/20 (no venue split)
    OR >=70% if n > 10.
  - H2H: if available, at least 1 of last 2 meetings hits the threshold.
  - Price gate: >= 1.72 (decimal), overs only.
  - Min sample: 6 games for team; opponent series can be shorter but must meet gates.
...
Outputs:
  data/value_bets/V4 Bets/team_run3_shots_sot.txt
...
OUT_FILE = ROOT / "data" / "value_bets" / "V4 Bets" / "team_run3_shots_sot.txt"
...
TEAM_MED_WINDOWS = [(9, 6), (10, 6), (12, 8), (14, 9), (17, 11), (20, 12)]
OPP_STRONG_WINDOWS = [(9, 8), (10, 8), (12, 10), (13, 11), (14, 12), (15, 13), (19, 15), (20, 16)]

TEAM_MARKETS = {
    "shots": "team shots",
    "sot": "team shots on target",
}
...
def parse_line(total: Optional[str]) -> Optional[float]:
    if not total:
        return None
    m = re.search(r"([0-9]+(?:\\.[0-9]+)?)", str(total).replace(",", "."))
    if not m:
        return None
    try:
...
def best_opp_gate(seq: List[int], threshold: int) -> Tuple[bool, float, int, int]:
    ok, best_ratio, best_hits, best_win = best_gate(seq, threshold, OPP_STRONG_WINDOWS)
    if len(seq) > 10:
        n = min(len(seq), 20)
...
def under_half_line_recent(seq: List[int], threshold: int) -> bool:
    """
    Drop if the team has been at or below 50% of the line in at least 2 of the last 10.
    """
    if not seq:
        return False
...
def series_any_venue(team_rec: dict, stat_key: str) -> List[int]:
    vals = team_rec.get(stat_key) or []
    locs = team_rec.get("locations_last_n") or []
    out = []
    for v, _ in zip(vals, locs):
        try:
            out.append(int(v))
        except Exception:
            continue
...
def h2h_last_same_venue(
    ...
    """
    Use the most recent H2H with the same home/away setup and return the stat
    value for the given team at that venue.
    """
    for fx in h2h_list:
        ...
        vec = ((fx.get("vectors") or {}).get(side) or {}).get(stat) or []
        for idx, meta in enumerate(metas):
            actual_home = meta.get("actual_home_id") or meta.get("home_id")
            actual_away = meta.get("actual_away_id") or meta.get("away_id")
            if int(actual_home or 0) == target_home and int(actual_away or 0) == target_away:
                if idx < len(vec):
                    try:
                        return int(vec[idx])
                    except Exception:
...
def best_over_rows(rows: List[dict], market_desc: str, side: str, price_gate: float) -> List[Tuple[float,float,dict]]:
    out = []
    for r in rows:
        if norm(r.get("market_description")) != market_desc:
            continue
        if label_to_side(r.get("label")) != side:
            continue
        total = r.get("total") or ""
        if "over" not in str(total).lower():
            continue
...
def process_fixture(
    ...
    for side, team_id, opp_id in [
        ...
        for stat, market_desc in TEAM_MARKETS.items():
            stat_key = "shots_on_target_last_n" if stat == "sot" else "shots_total_last_n"
            team_series = series_any_venue(team_rec, stat_key)
            team_sample = "overall"
            opp_series = series_any_venue(opp_rec, f"opp_{'shots_on_target' if stat=='sot' else 'shots_total'}_last_n")
            opp_sample = "overall"
            if len(team_series) < 6:
                drop("team sample <6")
                continue

            over_rows = best_over_rows(odds_rows, market_desc, side, MIN_PRICE)
            if not over_rows:
            ...
            for line, price, row in over_rows:
                ...
                ok_team, team_ratio, team_hits, team_win = best_gate(team_series, threshold, TEAM_MED_WINDOWS)
                if not ok_team:
                    continue
                if under_half_line_recent(team_series, threshold):
                    drop("team under half-line twice in last10", {"fixture": fx_meta.get("name"), "team": team_id, "line": line, "stat": stat})
                    continue
...
def format_pick(p: dict) -> str:
    stat_label = "shots on target" if p.get("stat") == "SOT" else "shots"
...
def main():
    ...
    for lid in league_ids:
        ...
        for fx in odds_blob.get("fixtures") or []:
            ...
            process_fixture(meta, odds_rows, team_stats, opp_stats, h2h_map, drops, picks, home_ml, away_ml)
    ...
    lines.append(f"Team (medium) windows: {', '.join([f'{req}/{win}' for win,req in TEAM_MED_WINDOWS])}")
    lines.append(f"Opponent (strong) windows: {', '.join([f'{req}/{win}' for win,req in OPP_STRONG_WINDOWS])} or >=70% if n>10")
    lines.append("")
    for p in picks:

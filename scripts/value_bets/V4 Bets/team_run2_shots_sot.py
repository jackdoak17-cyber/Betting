...
# -*- coding: utf-8 -*-
"""
Team Bets V4 — BET RUN 2 (Team shots & shots on target overs)

Logic (overs only, price gate >= 1.72):
 - Use same-venue samples (home/away) for team attack (shots, SOT).
 - Use opponent conceded samples (opp shots conceded, opp SOT conceded) at their
   upcoming venue.
 - Require strong team hit rates: any of {8/9, 8/10, 10/12, 11/13, 12/14,
   13/15, 15/19, 16/20}.
 - Require opponent concede signals: any of {4/5, 5/5, 6/8, 7/10} or >=70% if n>10.
 - Require at least 1 of the last 2 H2H (any venue) to have landed on the same
   threshold for that team (when available).
 - Skip lines with price < 1.72. Overs only.
...
Outputs:
  data/value_bets/V4 Bets/team_run2_shots_sot.txt
...
OUT_FILE = ROOT / "data" / "value_bets" / "V4 Bets" / "team_run2_shots_sot.txt"
...
def best_gate(seq: List[int], threshold: int, gates: List[Tuple[int, int]]) -> Tuple[bool, float, int, int]:
    """
    Evaluate hit-rate gates and return (ok, best_ratio, hits, window) where best_ratio
    is the highest ratio among windows that meet the gate.
    """
    ok = False
    best_ratio = -1.0
    best_hits = 0
...
def under_half_line_recent(seq: List[int], threshold: int) -> bool:
    ...
    E.g., line 10 -> 5 or fewer, line 9 -> 4 or fewer (floor).
    """
    if not seq:
        return False
...
def same_venue_series(team_rec: dict, stat_key: str, target_loc: Optional[str]) -> List[int]:
    """
    If target_loc is None, ignore venue and return the last 20 values.
    """
    ...
    for side, team_id, opp_id in [
        ...
        for stat, market_desc in TEAM_MARKETS.items():
            ...
            opp_series = same_venue_series(
                opp_rec,
                f"opp_{'shots_on_target' if stat=='sot' else 'shots_total'}_last_n",

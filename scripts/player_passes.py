...
# -*- coding: utf-8 -*-
"""
Build per-player passes histories (last N league matches of CURRENT season, ≥MIN_MINUTES).
Targets ONLY players found in data/predicted_xi/by_league/*.json.

Writes:
  - data/player_passes/by_league/{league_id}.json
  - data/player_passes/combined.json
  - data/player_passes/summary.txt

Notes:
- Sportmonks v3 type IDs used here:
  * PASSES = 80
  * MINUTES_PLAYED = 119
"""
...
# ---- Stat type ids ----
PASSES = 80           # Passes (Sportmonks team/player detail type)
...
OUT_ROOT = Path("data/player_passes")
...
def fetch_team_fixtures_window(team_id: int, start: dt.date, end: dt.date, league_id: int, page: int = 1) -> dict:
    ...
    Only lineups.details for PASSES + MINUTES.
    """
    path = f"fixtures/between/{dstr(start)}/{dstr(end)}/{team_id}"
    params = {
        "include": "lineups.details;lineups.player;state",
        "filters": f"lineupDetailTypes:{PASSES},{MINUTES_PLAYED};fixtureLeagues:{league_id}",
        "order": "desc",
        "per_page": 50,
        "page": page,
    }
...
def passes_from_details(details: List[dict]) -> int:
    for d in details or []:
        if int(d.get("type_id") or 0) == PASSES:
...
def collect_player_series_for_team(league_id: int, season_id: int, team_id: int, targets: Dict[int, dict]) -> Dict[int, dict]:
    """
    Returns: player_id -> { 'passes': [latest->older], 'minutes': [...], 'fixtures': [ids] }
    (Only fixtures in this league & season; include only if minutes >= MIN_MINUTES)
    """
    want = set(targets.keys())
    series: Dict[int, dict] = {pid: {"passes": [], "minutes": [], "fixtures": []} for pid in want}

    start_season, end_today = get_season_bounds(season_id)
    end = end_today
    have_all = lambda: all(len(series[pid]["passes"]) >= LAST_N for pid in want)
            ...
            for fx in data:
                ...
                for lp in fx.get("lineups") or []:
                    ...
                    if mins is None or mins < MIN_MINUTES:
                        continue
                    pval = passes_from_details(details)
                    s = series[pid]
                    s["passes"].append(int(pval))
    ...
    for pid in series:
        series[pid]["passes"] = series[pid]["passes"][:LAST_N]
    ...
    for idx, ((league_id, season_id, team_id), targets) in enumerate(items, 1):
        ...
        for pid, s in series.items():
            entry = {
                ...
                "passes_last_n": s["passes"],   # latest first

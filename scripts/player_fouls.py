...
# -*- coding: utf-8 -*-
"""
Build per-player fouls histories (last N league matches of CURRENT season, ≥45').
Targets ONLY players found in data/predicted_xi/by_league/*.json.
Writes:
  - data/player_fouls/by_league/{league_id}.json
  - data/player_fouls/combined.json
  - data/player_fouls/summary.txt

Notes:
- Uses Sportmonks v3 football:
  * fixtures between (team, date range) + include=lineups.details;lineups.player;state
  * filters=lineupDetailTypes:56,119;fixtureLeagues:{league_id}
  * Walk season in 100-day windows (API constraint).
- Stat type IDs:
  * FOULS=56, MINUTES_PLAYED=119 (Sportmonks Types).  # docs-backed
"""
...
OUT_ROOT = Path("data/player_fouls")
...
def fetch_team_fixtures_window(team_id: int, start: dt.date, end: dt.date, league_id: int, page: int = 1) -> dict:
    ...
    Only lineups.details for FOULS + MINUTES.
    """
    path = f"fixtures/between/{dstr(start)}/{dstr(end)}/{team_id}"
    params = {
        "include": "lineups.details;lineups.player;state",
        "filters": f"lineupDetailTypes:{FOULS},{MINUTES_PLAYED};fixtureLeagues:{league_id}",
        "order": "desc",
        "per_page": 50,
        "page": page,
    }
...
def fouls_from_details(details: List[dict]) -> int:
    for d in details or []:
        if int(d.get("type_id") or 0) == FOULS:
...
def collect_player_series_for_team(league_id: int, season_id: int, team_id: int, targets: Dict[int, dict]) -> Dict[int, dict]:
    """
    Returns: player_id -> { 'fouls': [latest->older], 'minutes': [...], 'fixtures': [ids] }
    (Only fixtures in this league & season; include only if minutes >= MIN_MINUTES)
    """
    want = set(targets.keys())
    series: Dict[int, dict] = {pid: {"fouls": [], "minutes": [], "fixtures": []} for pid in want}

    start_season, end_today = get_season_bounds(season_id)
    end = end_today
    have_all = lambda: all(len(series[pid]["fouls"]) >= LAST_N for pid in want)
            ...
            for fx in data:
                ...
                for lp in fx.get("lineups") or []:
                    ...
                    if mins is None or mins < MIN_MINUTES:
                        continue
                    fouls = fouls_from_details(details)
                    s = series[pid]
                    s["fouls"].append(int(fouls))
    ...
    for pid in series:
        series[pid]["fouls"] = series[pid]["fouls"][:LAST_N]
    ...
    for idx, ((league_id, season_id, team_id), targets) in enumerate(items, 1):
        ...
        for pid, s in series.items():
            entry = {
                ...
                "fouls_last_n": s["fouls"],      # latest first

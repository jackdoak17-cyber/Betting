...
- data/player_tackles/by_league/*.json   (expects 'players' with 'tackles_last_n')
and team names from:
- data/predicted_xi/by_league/{league_id}.json

Writes:
- data/player_tackles/summary_by_team.txt
"""
import json, glob
...
OUT_PATH = ROOT / "data" / "player_tackles" / "summary_by_team.txt"
...
def main():
    lines: List[str] = []
    for path in sorted(glob.glob(str(TACK_DIR / "*.json")), key=lambda p: int(Path(p).stem)):

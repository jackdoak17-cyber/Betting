...
Filters:
  ...
  - Player appears once (best price, highest-priority bucket)

Diagnostics printed to console and written to output header.
"""
...
HTTP_HEADERS = {"accept": "application/json", "user-agent": "odds-shots-value-singles/1.1"}
TIMEOUT = 25
...
# Common alias expansions to help team mapping
TEAM_ALIAS = {
...
def collect_candidates() -> List[dict]:
    ...
    for lid in discover_leagues():
        ...
        for rec in players:
            ...
            q55 = (hits5 == 5); q57 = (hits7 >= 5); q45 = (hits5 >= 4)
...
def main():
    ...
    # Events per league (optionally time-windowed)
    ...
    # Scan markets — best captured price per player (>= MIN_CAPTURE_PRICE), then filter by MIN_PRICE later
    ...
    for c in candidates:
        ...
        for ev_id in (c.get("event_ids") or []):
            ...
            # Team ML (with fallback)
            home_ml, away_ml, used_fb = min_win_prices(ev)
    ...
    # Apply your ≥ MIN_PRICE
    eligible = {k:v for k,v in best_by_player_all.items() if v["price"] >= MIN_PRICE}

    # Assign buckets (5/5 > 5/7 > 4/5). Player appears once total.
    ...
    sub = "Market: Bet365 — Player Shots Over 0.5 only. Player appears once (best price; highest-priority bucket)."

...
# -*- coding: utf-8 -*-
"""
Build per-player fouls **drawn** histories (last N league matches of CURRENT season, ≥45').
Targets ONLY players found in data/predicted_xi/by_league/*.json.

Writes:
  - data/player_fouls_drawn/by_league/{league_id}.json
  - data/player_fouls_drawn/combined.json
  - data/player_fouls_drawn/summary.txt

Notes:
- Sportmonks v3 football:
  * fixtures between (team, date range) + include=lineups.details;lineups.player;state
  * filters=lineupDetailTypes:96,119;fixtureLeagues:{league_id}
  * Walk season in 100-day windows (API constraint).
"""
...
# ---- Collection rules ----
LAST_N = int(os.getenv("PLAYER_FOULS_DRAWN_LAST_N", os.getenv("PLAYER_FOULS_LAST_N", "10")))
MIN_MINUTES = int(os.getenv("PLAYER_MIN_MINUTES", "45"))

# ---- IO ----
PRED_XI_DIR = Path("data/predicted_xi/by_league")
OUT_ROOT = Path("data/player_fouls_drawn")
...
def load_predicted_targets() -> Dict[Tuple[int, int, int], Dict[int, dict]]:
    ...
    Prefers detailed role tag from predicted XI if present.
    """
...
def fetch_team_fixtures_window(team_id: int, start: dt.date, end: dt.date, league_id: int, page: int = 1) -> dict:
    """
    GET fixtures for team in [start,end] with league filter & needed includes, ordered desc.
    Only lineups.details for FOULS_DRAWN + MINUTES.
    """
    path = f"fixtures/between/{dstr(start)}/{dstr(end)}/{team_id}"
    params = {
        "include": "lineups.details;lineups.player;state",
        "filters": f"lineupDetailTypes:{FOULS_DRAWN},{MINUTES_PLAYED};fixtureLeagues:{league_id}",
        "order": "desc",
        "per_page": 50,
        "page": page,
    }
...
def fouls_drawn_from_details(details: List[dict]) -> int:
    for d in details or []:
        if int(d.get("type_id") or 0) == FOULS_DRAWN:
...
def collect_player_series_for_team(league_id: int, season_id: int, team_id: int, targets: Dict[int, dict]) -> Dict[int, dict]:
    """
    Returns: player_id -> { 'fouls_drawn': [latest->older], 'minutes': [...], 'fixtures': [ids] }
    (Only fixtures in this league & season; include only if minutes >= MIN_MINUTES)
    """
    want = set(targets.keys())
    series: Dict[int, dict] = {pid: {"fouls_drawn": [], "minutes": [], "fixtures": []} for pid in want}

    start_season, end_today = get_season_bounds(season_id)
    end = end_today

    def have_all() -> bool:
        return all(len(series[pid]["fouls_drawn"]) >= LAST_N for pid in want)
            ...
            for fx in data:
                ...
                for lp in fx.get("lineups") or []:
                    ...
                    if mins is None or mins < MIN_MINUTES:
                        continue
                    fdrawn = fouls_drawn_from_details(details)
                    s = series[pid]
                    s["fouls_drawn"].append(int(fdrawn))
    ...
    for pid in series:
        series[pid]["fouls_drawn"] = series[pid]["fouls_drawn"][:LAST_N]
...
def main():
    ...
    for idx, ((league_id, season_id, team_id), targets) in enumerate(items, 1):
        ...
        for pid, s in series.items():
            entry = {
                ...
                "fouls_drawn_last_n": s["fouls_drawn"],  # latest first
    ...
    for lid, rows in per_league_players.items():
        ...
        payload = {"generated_at": generated_at, "league_id": lid, "count": len(rows), "players": rows}
    ...
    # combined
    combined = {"generated_at": generated_at, "count": len(combined_players), "players": combined_players}
    ...
    # summary
    lines = [f"Time (UTC): {generated_at}", f"Players : {len(combined_players)}", ""]
